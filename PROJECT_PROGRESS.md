...
- 臨時 DEBUG 診斷工具（`src/features/debug/DataInventoryDebug.tsx`，掛在統計頁底部，**移除延後至 Phase C6**：刪除同步驗收時要用它確認「刪除後筆數減少」）：統計 7 張 IndexedDB 表筆數、`noteImages` 圖片總數/總大小/單張最大值、非圖片資料的 JSON 序列化大小估計
...
- Phase C0 的臨時 DEBUG 資料盤點工具**暫緩移除**：C6 要做刪除同步，這個工具對驗證「刪除後筆數減少」還有用，留到 C6 驗收完再整段移除（設計上已集中單一資料夾，屆時仍可一次移除乾淨）
