...
- 從網路（`fetch`／Supabase Storage `.download()` 等）拿到的 `Blob`，存進 IndexedDB（`db.table.add()`）前一律先用 `new Blob([await blob.arrayBuffer()], { type: blob.type })` 重新包裝，不可把 fetch 回傳的 Blob 原樣寫入。原因：Phase C4b 圖片下載在 Chrome 無痕視窗實測噴 `DexieError: UnknownError: Error preparing Blob/File data to be stored in object store`——無痕視窗的 IndexedDB 後端無法對「直接由網路回應支撐」的 Blob 做 structured clone，`.list()`（純 JSON）與 `.download()`（網路層本身）都正常，只有寫入 IndexedDB 那一步失敗，且原本每筆圖片的 try/catch 是全靜默的，導致「失敗」與「根本沒下載」從外部完全無法區分（連帶教訓：任何「單筆失敗不擋其他筆」的 try/catch，都要留一行 `console.warn` 記錄失敗原因與識別資訊，靜默吞錯會讓真正的 bug 偽裝成「沒發生任何事」，難以定位）。透過 `arrayBuffer()` 重新包裝成純記憶體 Blob 之後即可正常寫入，一般視窗本來就不受影響，但這個修法沒有下行成本，之後任何「fetch 來的 Blob 要進 IndexedDB」都應該預設套用，不要等下一次無痕視窗回報才修。
- 測試全過但真機仍然壞掉時，優先懷疑測試的 mock 只驗到「呼叫了某個 API」，沒驗到「該 API 真正產生的副作用有沒有發生」。原因：Phase C4b 圖片下載的既有測試只斷言 `noteImages` 表最終出現一筆資料，這條斷言剛好在 mock 環境下能通過，但沒有專門斷言「`.download()` 這個 mock 真的被呼叫、且回傳的 blob 真的走到 `db.noteImages.add()`」——導致「下載步驟被跳過」與「Blob 寫入 IndexedDB 失敗」這兩種完全不同的真機錯誤，在 mock 環境下都可能被同一條寬鬆斷言蓋過去而測不出來。日後補測試時，要對照「真機真正失敗的那個步驟」直接下斷言（例如明確 assert mock 函式被以特定參數呼叫），不能只驗最終結果的形狀。
- 驗到壞結果、且錯誤內容與修正前完全相同時，先確認「測到的是不是最新版」，再懷疑程式碼本身有沒有修對——本專案已多次發生「改了但沒生效」而非「改錯」。判斷法：`git log --oneline -1` 對照本機最新 commit、`ps`/`lsof` 查 dev server process 的啟動時間是否早於該 commit 的 commit 時間（`git log --format="%ad" --date=iso`），啟動時間早於修正 commit 就代表這個 server process 可能沒吃到最新程式碼，應先關閉重開，而不是直接改程式碼找「另一個 bug」。原因：Phase C4b 的 Blob 修正一度被誤判為無效，追查後發現使用者連接的 dev server（port 5173）啟動時間比修正 commit 還早，且背景還同時跑著另一個更舊的 server（port 5174）容易搞混——這是與「PWA 正式站 Service Worker 快取」同一類「未確認吃到最新版就先懷疑程式碼」的教訓，但這次的變因是本機 dev server 的 process 存活時間，不是正式站的 SW。
